...
def verify_rollback_receipt(
    ...
    * the lineage spine verifies and contains an entry whose content hash
    ...
    if not report.ok:
        ...
        return RollbackVerifyResult(ok=False, reason=f"evolution-rollback spine failed verification: {detail}", receipt=receipt)
    ...
    if not anchored:
        return RollbackVerifyResult(ok=False, reason="receipt is not anchored in the evolution-rollback spine", receipt=receipt)
