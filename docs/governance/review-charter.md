...
- **Small sensitive changes.** A change of at most 400 changed lines on a path naming `sandbox`, `security` or `audit` outside `tests/` and `docs/`, or under `src/bernstein/core/identity/` or `src/bernstein/core/tokens/`, meets its approval requirement with the maintainer's approval on its current head, one core reviewer's approval on that head, green CI, no standing changes requested, and 72 hours since the later of those two approvals. Above 400 lines the approvals this section already asks for remain.
