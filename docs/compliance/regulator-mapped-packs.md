...
- **Evidence hrefs are pack-relative.** Each observation's `relevant-evidence` href is `bench-bundles/<bundle-hash>.json` -- the bundle's filename *as it appears inside an evidence pack*. The export runs standalone (`--workdir`, no pack), so the href resolves only when the reader also builds a pack from the same `.sdd`; it is a stable reference to the bundle, not a local file path.
- **The document is content-addressed, dates included.** `metadata.published`, `metadata.last-modified` and the result's `start` are the latest bundle's `submitted_at`, the same rule each observation's `collected` follows -- so two exports of the same assessment are byte-identical. With no bundles to date the fields fall back to the Unix epoch.
